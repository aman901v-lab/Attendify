...
import { UserSettings, Holiday, AppTheme, User, Feedback } from '../types';
import { 
...
// Ensure correct modular syntax for Firestore functions
...
const SettingsView: React.FC<Props> = ({ settings, onSave, currentUser, onUpdateUser, onUpdateAvatar }) => {
  ...
  // Master logic: check role OR master username (aman.verma)
      ...
      <div className="glass p-6 rounded-[2.5rem] border border-white/5 space-y-6 shadow-2xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5 opacity-60">
            <label className="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Full Name (Locked)</label>
          ...
          <div className="space-y-1.5 opacity-60">
            <label className="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Username (Locked)</label>
          ...
          <div className="space-y-1.5 opacity-80">
            <label className="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest flex items-center gap-1"><Hash size={10} className="text-amber-400" /> Calculated Age</label>
            <div className="w-full bg-slate-900/40 border border-white/5 rounded-2xl px-5 py-4 text-slate-300 text-sm font-bold flex items-center justify-between">
              <span>{currentCalculatedAge > 0 ? `${currentCalculatedAge} Years Old` : 'Please select DOB'}</span>
              <span className="text-[8px] bg-white/5 px-2 py-1 rounded text-slate-500 font-black uppercase tracking-widest">Auto-Generated</span>
            </div>
          </div>
          <div className="space-y-1.5">
        ...
        <button onClick={handleUpdateProfile} className="w-full py-4 bg-sky-500 hover:bg-sky-600 text-white font-black rounded-2xl transition-all active:scale-95 uppercase tracking-widest text-[10px] shadow-xl shadow-sky-500/20">
