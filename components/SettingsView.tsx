...
const SettingsView: React.FC<Props> = ({ settings, onSave, currentUser, onUpdateUser, onUpdateAvatar }) => {
  ...
  const startVerification = (type: 'email' | 'phone') => {
    ...
    // In a production app, this would trigger an API call to Twilio/SendGrid.
    // Here we use an alert to show the code to the user since it's a frontend demo.
    setTimeout(() => {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 animate-in fade-in duration-500">
      ...
      <div className="flex-1 min-w-0 min-h-[500px]">
        ...
        {/* ... Rest of the component tabs (system, holidays, feedback) remain optimized ... */}
              ...
              {localSettings.holidays.map((h, i) => (
                <div key={i} className="flex justify-between items-center p-4 bg-white/5 rounded-xl border border-white/5">
                  <div className="flex items-center gap-3">
                    ...
                    <div><p className="text-xs font-bold text-white">{h.name}</p><p className="text-[9px] text-slate-500 uppercase">{h.date}</p></div>
                  </div>
      ...
      {/* Verification Modal (Simulated Real Logic) */}
      {verificationModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-950/95 backdrop-blur-2xl">
          <div className="relative glass w-full max-w-sm p-10 rounded-[3rem] border border-sky-500/30 text-center space-y-6 shadow-[0_0_100px_rgba(14,165,233,0.1)]">
            ...
            <div className="p-4 bg-amber-500/5 border border-amber-500/10 rounded-2xl flex items-start gap-3 text-left">
              ...
              <p className="text-[8px] text-amber-500/70 font-bold leading-relaxed uppercase">Demo Mode: In a production system, this code would be sent via real SMS/Email. Please check the alert box on your screen for the code.</p>
            </div>
          </div>
        </div>
      )}
...
const ProfileItem = ({ label, icon: Icon, value, isEditing, verified, type = "text", onVerify, onChange }: any) => (
  <div className="space-y-2 group">
    ...
    {isEditing ? (
      <input type={type} value={value} onChange={e => onChange(e.target.value)} className="w-full bg-slate-900 border border-white/5 rounded-xl px-4 py-3 text-white text-xs focus:border-sky-500 outline-none transition-all shadow-inner" />
    ) : (
