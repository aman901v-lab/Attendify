...
const StaffManager: React.FC<Props> = ({ 
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    phone: '',
    salaryHistory: [] as { month: string; salary: number }[]
  });
  ...
  // Fix: Implemented toggleTab to handle module visibility selection
  const toggleTab = (tabId: string) => {
    setFormData(prev => ({
      ...prev,
      accessibleTabs: prev.accessibleTabs.includes(tabId)
        ? prev.accessibleTabs.filter(id => id !== tabId)
        : [...prev.accessibleTabs, tabId]
    }));
  };

  // Fix: Implemented toggleDay to handle weekly off selection
  const toggleDay = (dayIdx: number) => {
    setFormData(prev => ({
      ...prev,
      weeklyOffs: prev.weeklyOffs.includes(dayIdx)
        ? prev.weeklyOffs.filter(d => d !== dayIdx)
        : [...prev.weeklyOffs, dayIdx].sort()
    }));
  };
  ...
  const handleEdit = (emp: Employee) => {
    ...
    setFormData({
      ...
      // Fix: Strictly prioritize saved otRate. Only use '200' if it's literally missing (undefined)
      otRate: (emp.otRate !== undefined && emp.otRate !== null) ? emp.otRate.toString() : '200',
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  ...
                  <input type="number" value={formData.otRate} onChange={e => setFormData({...formData, otRate: e.target.value})} className="w-full bg-slate-950 border border-white/10 rounded-2xl px-5 py-4 text-white text-sm outline-none focus:border-sky-500" placeholder="OT/Hr" />
              ...
              {/* Salary Increment History Manager */}
              <div className="bg-slate-900/50 p-6 rounded-3xl border border-white/5 space-y-4">
                <div className="flex items-center justify-between border-b border-white/5 pb-2">
                  <div className="flex items-center gap-2 text-sky-400">
                    <TrendingUp size={16} />
                    <h4 className="text-[10px] font-black uppercase tracking-widest">Salary Increment History</h4>
                  </div>
                </div>
                
                <p className="text-[10px] text-slate-500 leading-relaxed">
                  Add custom salaries for specific effective months. The system will automatically show the updated salary from that month onwards, and keep the original salary for any month prior in report.
                </p>

                {/* Add new increment fields */}
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end bg-slate-950 p-4 rounded-2xl border border-white/5">
                  <div className="space-y-1">
                    <label className="text-[8px] font-black text-slate-500 uppercase ml-1">Effective Month</label>
                    <input 
                      type="month" 
                      id="increment_month"
                  ...
                  <div className="space-y-1">
                    <label className="text-[8px] font-black text-slate-500 uppercase ml-1">New Salary (₹)</label>
                    <input 
                      type="number" 
                      id="increment_salary"
                  ...
                  <button 
                    type="button"
                    onClick={() => {
                      const monthInput = document.getElementById('increment_month') as HTMLInputElement;
                      const salaryInput = document.getElementById('increment_salary') as HTMLInputElement;
                      if (monthInput && salaryInput && monthInput.value && salaryInput.value) {
                        const newInc = {
                          month: monthInput.value,
                          salary: Number(salaryInput.value)
                        };
                        setFormData(prev => {
                          const filtered = (prev.salaryHistory || []).filter(h => h.month !== newInc.month);
                          return {
                            ...prev,
                            salaryHistory: [...filtered, newInc].sort((a, b) => a.month.localeCompare(b.month))
                          };
                        });
                        monthInput.value = '';
                        salaryInput.value = '';
                      } else {
                        alert("Please select both Month and Salary.");
                      }
                    }}
                    className="w-full bg-sky-500/10 hover:bg-sky-500 hover:text-white border border-sky-500/20 text-sky-400 py-3 rounded-xl font-bold text-xs transition-all uppercase tracking-wider"
                  >
                    Add Increment
                  </button>
                </div>

                {/* List of existing increments */}
                {formData.salaryHistory && formData.salaryHistory.length > 0 ? (
                  <div className="space-y-2 mt-2">
                    <label className="text-[8px] font-black text-slate-500 uppercase ml-1">Configured Increments:</label>
                    <div className="divide-y divide-white/5 bg-slate-950 rounded-2xl border border-white/5 max-h-[150px] overflow-y-auto">
                      {formData.salaryHistory.map((h, i) => (
                        <div key={i} className="flex justify-between items-center px-4 py-3 text-xs">
                          <div className="flex items-center gap-4">
                            <span className="font-mono text-white/80 bg-white/5 px-2 py-1 rounded">{h.month}</span>
                            <span className="font-bold text-emerald-400">₹{h.salary.toLocaleString()}</span>
                          </div>
                          <button 
                            type="button" 
                            onClick={() => {
                              setFormData(prev => ({
                                ...prev,
                                salaryHistory: (prev.salaryHistory || []).filter(item => item.month !== h.month)
                              }));
                            }}
                            className="text-rose-500 hover:text-rose-400 px-2 py-1 hover:bg-rose-500/10 rounded-lg transition-all text-[10px] font-bold"
                          >
                            Delete
                          </button>
                        </div>
                  ...
                  <p className="text-[10px] text-slate-600 italic pl-1">No custom salary increments configured yet.</p>
                )}
              </div>
