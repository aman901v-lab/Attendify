...
const StaffManager: React.FC<Props> = ({ 
  ...
  const [formData, setFormData] = useState({
    ...
    otRate: '200',
    dailyHours: '8.5',
    workingDays: '26', // STRICT DEFAULT 26
    sl: '12', 
    pl: '15', 
    cl: '10',
    notes: '',
    ...
    <div className="space-y-6 min-h-[500px] w-full">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/5 p-5 rounded-[2rem] border border-white/5 shadow-lg">
        <div>
          ...
          <p className="text-slate-500 text-[10px] uppercase font-bold tracking-widest mt-0.5">Manage access & roles</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-20">
            ...
            return (
              <div key={emp.id} className="glass p-5 rounded-[2rem] border border-white/5 flex items-center justify-between group hover:border-sky-500/30 transition-all shadow-xl bg-gradient-to-br from-white/5 to-transparent">
                ...
                <div className="flex items-center gap-2 ml-4">
                  ...
                  <button onClick={() => { if(window.confirm(`Delete ${emp.name}?`)) onDelete(emp.id); }} className="p-3 bg-rose-500/10 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-all border border-rose-500/20"><Trash2 size={16} /></button>
                </div>
              </div>
            );
        })}
        {employees.length === 0 && (
          <div className="md:col-span-2 glass-dark p-20 rounded-[3rem] border border-white/5 text-center flex flex-col items-center gap-6 bg-white/5">
            <div className="p-6 bg-white/5 rounded-full text-slate-700 shadow-inner"><UserIcon size={48} /></div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="glass-dark p-6 rounded-[2rem] border border-white/5 space-y-6">
                <div className="flex items-center justify-between border-b border-white/5 pb-4">
                  ...
                  >
                    {formData.isAdmin ? 'Admin Status' : 'Staff Status'}
                  </button>
                </div>
                ...
                {!formData.isAdmin && (
                  <div className="space-y-3 pt-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Limit Access To:</label>
              ...
              <div className="space-y-5">
                <div className="flex items-center gap-2 text-amber-400 border-b border-white/5 pb-2">
                  <Briefcase size={16} />
                  <h4 className="text-[10px] font-black uppercase tracking-[0.2em]">Job Details</h4>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Monthly Salary</label>
                    <div className="relative">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div className="space-y-2 col-span-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Work Cycle (Fixed to 26)</label>
                    <select 
                      value={formData.workingDays} 
                      onChange={e => setFormData({...formData, workingDays: e.target.value})} 
                      className="w-full bg-slate-900 border border-sky-500/30 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-sky-500 appearance-none text-sm shadow-inner font-bold"
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="number" value={formData.otRate} onChange={e => setFormData({...formData, otRate: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-2xl px-5 py-4 text-white text-sm" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-tighter">Shift (H)</label>
                    <input type="number" step="0.5" value={formData.dailyHours} onChange={e => setFormData({...formData, dailyHours: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-2xl px-5 py-4 text-white text-sm" />
                  </div>
                </div>
              ...
              <div className="pt-4">
                <button type="submit" className="w-full py-5 bg-sky-500 hover:bg-sky-600 text-white font-black rounded-[1.5rem] shadow-2xl shadow-sky-500/30 transition-all active:scale-95 flex items-center justify-center gap-3 uppercase tracking-[0.2em] text-xs">
                  <Check size={20} /> {editingEmployee ? 'Update' : 'Register'}
                </button>
              </div>
            </form>
          </div>
