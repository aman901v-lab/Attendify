...
const StaffManager: React.FC<Props> = ({ employees, users, globalWeeklyOffs, onAdd, onUpdate, onDelete }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Find if user already exists or create new
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 mb-2 text-amber-400">
                  <Briefcase size={16} />
                  <h4 className="text-xs font-bold uppercase tracking-widest">Job Configuration (26-Day Cycle)</h4>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input type="number" value={formData.salary} onChange={e => setFormData({...formData, salary: Number(e.target.value)})} className="w-full bg-slate-900 border border-white/10 rounded-xl pl-8 pr-4 py-3 text-white" />
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input type="number" value={formData.otRate} onChange={e => setFormData({...formData, otRate: Number(e.target.value)})} className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white" />
                  </div>
                  <div className="space-y-1.5">
