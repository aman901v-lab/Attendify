...
import { calculateWorkedHours, getOT, getAutoStatus, toLocalDateString, formatTime12h, formatDate } from '../utils.ts';
...
const STATUS_CONFIG: Record<AttendanceStatus, { color: string, bg: string }> = {
  ...
  'Pending': { color: 'text-amber-400/80 animate-pulse', bg: 'bg-amber-500/10' },
};
...
const Calendar: React.FC<Props> = ({ employee, records, holidays, onAddRecord, onDeleteRecord }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.status === 'Duty' || formData.status === 'Half-Day') {
      if (formData.checkIn && formData.checkOut) {
        const worked = calculateWorkedHours(formData.checkIn, formData.checkOut);
        const ot = getOT(worked);
        finalRecord = { 
          ...finalRecord, 
  ...
  const handleDelete = () => {
    if (selectedDate && window.confirm(`Permanently delete entry for ${formatDate(selectedDate)}?`)) {
          ...
          return (
            ...
            >
              ...
              {record && record.checkIn && (
                 <span className="absolute bottom-1 text-[5px] font-black uppercase text-sky-500/50">
                   {formatTime12h(record.checkIn).split(' ')[1]}
                 </span>
              )}
            </button>
          );
          ...
          <form onSubmit={handleSubmit} className="relative w-full max-w-md glass p-6 sm:p-8 rounded-[2.5rem] border border-sky-500/30 space-y-6 animate-in zoom-in duration-300">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-0.5">Edit entry for {formatDate(selectedDate || '')}</p>
              </div>
            ...
            {(formData.status === 'Duty' || formData.status === 'Half-Day') && (
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Punch In (24h)</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Punch Out (24h)</label>
