...
try {
  ...
  console.error("Critical rendering error:", error instanceof Error ? error.message : String(error));
  rootElement.innerHTML = `<div style="color: white; padding: 20px; text-align: center;">
    ...
    <pre style="font-size: 10px; opacity: 0.5;">${error instanceof Error ? error.message : String(error)}</pre>
  </div>`;
}