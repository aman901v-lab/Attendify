...
const Reports: React.FC<Props> = ({ employee, records, user }) => {
  ...
  const exportExcel = () => {
    let csvContent = "Date,In,Out,Status,Worked (Hrs),OT (Hrs),Daily Pay (INR)\n";
    monthlyRecords.forEach(r => {
      const pay = getDailyAmount(r);
      csvContent += `${r.date},${r.checkIn || '-'},${r.checkOut || '-'},${r.status},${r.totalHours || 0},${(r.otHours || 0).toFixed(2)},${Math.round(pay)}\n`;
    });
    
    csvContent += `\nSUMMARY,,,,,\n`;
    csvContent += `Base Salary,${employee.monthlySalary},,,,\n`;
    csvContent += `Total OT Pay,${Math.round(stats.otEarnings)},,,,\n`;
    csvContent += `Deductions,${Math.round(stats.deductions)},,,,\n`;
    csvContent += `NET PAYABLE,${Math.round(stats.netSalary)},,,,\n`;

    downloadCSV(csvContent, `${employee.name}_${selectedMonth}_Report.csv`);
  };
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="glass p-6 rounded-[2rem] border border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-black text-white uppercase tracking-tight">Report Center</h2>
            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Billing Period: {selectedMonth}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="glass p-8 rounded-[2.5rem] border border-white/5 bg-gradient-to-br from-white/5 to-transparent flex items-center justify-between shadow-xl">
           <div className="flex items-center gap-6">
            ...
            <div>
              <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">Net Payout</p>
          ...
          <table className="w-full text-left">
            <thead className="bg-white/5 border-b border-white/5">
              <tr>
                ...
                <th className="px-5 py-4 text-[9px] font-black text-slate-500 uppercase tracking-widest">In/Out</th>
                ...
                <th className="px-5 py-4 text-[9px] font-black text-slate-500 uppercase tracking-widest text-right">Day Pay</th>
              </tr>
            </thead>
      ...
      <div id="print-area" className="hidden print:block fixed inset-0 bg-white text-black z-[9999]">
        <div className="p-10 max-w-[210mm] mx-auto border-2 border-black">
            <div className="flex justify-between items-center border-b-4 border-black pb-6 mb-8">
               <div>
                  <h1 className="text-4xl font-black uppercase leading-none">Monthly Payslip</h1>
                  <p className="text-sm font-bold text-slate-600 mt-2">Period: {selectedMonth}</p>
               </div>
               <div className="text-right">
                  <h2 className="text-2xl font-black text-sky-600 tracking-tighter">ATTENDIFY CLOUD</h2>
            ...
            <table className="w-full text-left border-collapse border border-slate-300 mb-8">
                <thead>
                    <tr className="bg-slate-100">
                      ...
                      <th className="py-2 px-3 border border-slate-300 text-[10px] font-black uppercase">OT Hrs</th>
                      <th className="py-2 px-3 border border-slate-300 text-[10px] font-black uppercase text-right">Pay ₹</th>
                    </tr>
                </thead>
                <tbody>
                    {monthlyRecords.map(r => {
            ...
            <div className="grid grid-cols-2 gap-8 border-t-2 border-black pt-6">
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                    ...
                    <div className="flex justify-between text-sm text-rose-600"><span>Deductions:</span> <strong>-₹{Math.round(stats.deductions).toLocaleString()}</strong></div>
                </div>
                <div className="bg-sky-600 text-white p-8 rounded-2xl flex flex-col justify-center items-center">
            ...
            <div className="mt-16 flex justify-between border-t border-slate-200 pt-8 opacity-50">
                <p className="text-[8px] font-bold uppercase tracking-widest">Generated: {new Date().toLocaleString()}</p>
