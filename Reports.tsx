...
const Reports: React.FC<Props> = ({ employee, records, user }) => {
  ...
  // Sort and filter records for the selected month
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="glass p-6 rounded-[2rem] border border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Select billing cycle</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <button onClick={exportExcel} className="glass p-6 rounded-[2rem] border border-white/5 flex flex-col items-center gap-3 hover:bg-emerald-500/10 transition-all group">
           ...
           <span className="text-[10px] font-black text-white uppercase tracking-widest">Excel Report</span>
        </button>
        <button onClick={exportCSV} className="glass p-6 rounded-[2rem] border border-white/5 flex flex-col items-center gap-3 hover:bg-sky-500/10 transition-all group">
           ...
           <span className="text-[10px] font-black text-white uppercase tracking-widest">CSV Data</span>
        </button>
      ...
      <div className="glass p-8 rounded-[2.5rem] border border-white/5 bg-gradient-to-br from-white/5 to-transparent flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl">
        <div className="flex items-center gap-6">
          ...
          <div>
            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mb-1">Total Salary (EST.)</p>
            <h4 className="text-4xl font-black text-white tracking-tighter">₹{Math.round(stats.netSalary).toLocaleString()}</h4>
          </div>
        </div>
        <div className="text-center md:text-right space-y-1">
           <p className="text-[9px] text-slate-600 font-bold uppercase tracking-widest">Cycle Records</p>
           <p className="text-2xl font-black text-sky-400">{monthlyRecords.length} / {employee.workingDaysPerMonth || 26}</p>
        </div>
      </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-white/5 border-b border-white/5">
              <tr>
                ...
                <th className="px-6 py-4 text-[9px] font-black text-slate-500 uppercase tracking-widest">Duty Hrs</th>
                <th className="px-6 py-4 text-[9px] font-black text-slate-500 uppercase tracking-widest">OT</th>
                <th className="px-6 py-4 text-[9px] font-black text-slate-500 uppercase tracking-widest">Notes</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {monthlyRecords.map(r => (
                <tr key={r.date} className="hover:bg-white/5 transition-colors">
                  <td className="px-6 py-4 text-xs font-mono text-white/60">{r.date}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest ${r.status === 'Duty' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                      {r.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-xs font-bold text-white/70">{r.totalHours || '-'}</td>
                  <td className="px-6 py-4 text-xs font-bold text-amber-500">{r.otHours ? `+${r.otHours}` : '-'}</td>
                  <td className="px-6 py-4 text-[10px] text-slate-500 italic truncate max-w-[150px]">{r.notes}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* PRINT-ONLY TEMPLATE (PDF DESIGN) */}
      <div id="print-area" className="hidden print:block fixed inset-0 bg-white text-black p-12 z-[9999] overflow-visible">
        <div className="flex justify-between items-start border-b-4 border-black pb-8 mb-8">
           <div>
              <h1 className="text-4xl font-black uppercase leading-none">Monthly Statement</h1>
              <p className="text-lg font-bold text-slate-500 mt-2">{selectedMonth}</p>
           </div>
           <div className="text-right">
              <h2 className="text-2xl font-black text-sky-600">ATTENDIFY</h2>
              <p className="text-[10px] font-bold text-slate-400 tracking-[0.3em] uppercase">Cloud Node Reporting</p>
           </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-12 mb-10">
           <div className="space-y-1">
              ...
              <p className="text-lg"><strong>Emp ID:</strong> {employee.employeeId}</p>
        ...
        <table className="w-full text-left border-collapse mb-10">
           <thead>
              <tr className="border-y-2 border-black bg-slate-50">
                 <th className="py-4 px-3 font-black uppercase text-xs">Date</th>
                 <th className="py-4 px-3 font-black uppercase text-xs">Status</th>
                 <th className="py-4 px-3 font-black uppercase text-xs">Hrs</th>
                 <th className="py-4 px-3 font-black uppercase text-xs">OT</th>
                 <th className="py-4 px-3 font-black uppercase text-xs">Notes</th>
              </tr>
           </thead>
           <tbody>
              {monthlyRecords.map(r => (
                 <tr key={r.date} className="border-b border-slate-200">
                    <td className="py-3 px-3 text-xs font-mono">{r.date}</td>
                    <td className="py-3 px-3 text-xs font-bold uppercase">{r.status}</td>
                    <td className="py-3 px-3 text-xs">{r.totalHours || '-'}</td>
                    <td className="py-3 px-3 text-xs font-bold">{r.otHours ? `+${r.otHours}` : '-'}</td>
                    <td className="py-3 px-3 text-xs italic text-slate-500 truncate max-w-[200px]">{r.notes}</td>
                 </tr>
              ))}
           </tbody>
        </table>
        ...
        <div className="grid grid-cols-3 gap-8 bg-slate-50 p-10 rounded-3xl border border-slate-200 mt-auto">
           <div>
              <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Base Monthly</p>
           ...
           <div className="text-right">
              <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Net Payable</p>
        ...
        <div className="mt-20 flex justify-between pt-8 border-t border-slate-200 opacity-50">
           <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">SYSTEM VERIFIED DOCUMENT • GENERATED {new Date().toLocaleString()}</p>
           <p className="text-[9px] font-bold text-slate-400 uppercase">PAGE 1 / 1</p>
        </div>
      </div>
