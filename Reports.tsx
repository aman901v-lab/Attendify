...
const Reports: React.FC<Props> = ({ employee, records, user }) => {
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div id="print-area" className="hidden print:block bg-white text-black z-[9999]">
        <div className="max-w-[210mm] mx-auto bg-white">
            {/* Header */}
            <div className="flex justify-between items-start border-b-2 border-slate-900 pb-4 mb-6">
               <div>
                  <h1 className="text-2xl font-black text-slate-950 uppercase tracking-tighter leading-none mb-1">Salary Statement</h1>
                  <div className="flex items-center gap-2">
                    <span className="bg-slate-900 text-white text-[10px] font-black px-2 py-1 rounded uppercase tracking-widest">Official Record</span>
                    <span className="text-slate-500 text-[11px] font-bold">Billing Cycle: {selectedMonth}</span>
                  </div>
               </div>
               <div className="text-right">
                  ...
                  <p className="text-[9px] text-slate-400 font-bold">Gen: {new Date().toLocaleDateString('en-GB').replace(/\//g, '-')}</p>
               </div>
            </div>

            {/* Profile & Stats Bento */}
            <div className="grid grid-cols-2 gap-6 mb-6 page-break-inside-avoid">
               <div className="bg-slate-50 p-5 rounded-xl border border-slate-200">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Employee Identity</p>
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-[11px] font-bold">{employee.employeeId}</p>
                      </div>
                      <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase">Designation</p>
                        <p className="text-[11px] font-bold">{employee.role}</p>
                      </div>
                    </div>
                    <div className="pt-2">
                      <p className="text-[9px] font-black text-slate-400 uppercase">Organization</p>
                      <p className="text-[11px] font-bold text-sky-600 truncate">{user?.companyName || 'N/A'}</p>
                    </div>
                  </div>
               </div>
               <div className="bg-slate-50 p-5 rounded-xl border border-slate-200">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-4">
                     <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase">Cycle Days</p>
                        <p className="text-lg font-black leading-none">{employee.workingDaysPerMonth || 26}</p>
                     </div>
                     <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase">Present</p>
                        <p className="text-lg font-black text-emerald-600 leading-none">{stats.presentDays}</p>
                     </div>
                     <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase">OT Accrued</p>
                        <p className="text-lg font-black text-amber-500 leading-none">{stats.totalOTHours.toFixed(2)}</p>
                     </div>
                     <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase">Absents</p>
                        <p className="text-lg font-black text-rose-500 leading-none">{stats.absentDays + stats.halfDays * 0.5}</p>
                     </div>
                  </div>
               </div>
            </div>
            ...
            <div className="border border-slate-200 rounded-lg overflow-hidden mb-6 shadow-sm">
                <table className="w-full text-left border-collapse" style={{ tableLayout: 'fixed' }}>
                    <thead className="bg-slate-100 border-b border-slate-200">
                        <tr>
                          <th className="py-3 px-3 text-[10px] font-black text-slate-600 uppercase tracking-widest w-[16%]">Date</th>
                          <th className="py-3 px-3 text-[10px] font-black text-slate-600 uppercase tracking-widest w-[12%]">In</th>
                          <th className="py-3 px-3 text-[10px] font-black text-slate-600 uppercase tracking-widest w-[12%]">Out</th>
                          <th className="py-3 px-3 text-[10px] font-black text-slate-600 uppercase tracking-widest w-[16%]">Status</th>
                          <th className="py-3 px-3 text-[10px] font-black text-slate-600 uppercase tracking-widest text-center w-[14%]">Worked</th>
                          <th className="py-3 px-3 text-[10px] font-black text-slate-600 uppercase tracking-widest text-center w-[14%]">OT</th>
                          <th className="py-3 px-3 text-[10px] font-black text-slate-600 uppercase tracking-widest text-right w-[16%]">Total ₹</th>
                        </tr>
                    </thead>
                        ...
                        {monthlyRecords.map(r => {
                          ...
                          return (
                              <tr key={r.date} className="hover:bg-slate-50">
                                <td className="py-2 px-3 text-[11px] font-mono font-bold text-slate-600 break-words">{formatDate(r.date)}</td>
                                <td className="py-2 px-3 text-[11px] font-bold break-words">{formatTime12h(r.checkIn)}</td>
                                <td className="py-2 px-3 text-[11px] font-bold break-words">{formatTime12h(r.checkOut)}</td>
                                ...
                                <td className="py-2 px-3 text-[11px] font-bold text-center break-words">{(r.totalHours || 0).toFixed(2)}</td>
                                <td className="py-2 px-3 text-[11px] font-bold text-amber-600 text-center break-words">{(r.otHours || 0).toFixed(2)}</td>
                                <td className="py-2 px-3 text-[11px] font-black text-right text-slate-900 break-words">₹{Math.round(dailyTotal)}</td>
                              </tr>
                          );
                        })}
                    </tbody>
            ...
            {/* Earnings Final Card */}
            <div className="grid grid-cols-2 gap-6 pt-6 border-t-2 border-slate-900 page-break-inside-avoid">
                <div className="space-y-3">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Breakdown Detail</p>
                    <div className="space-y-2">
                    ...
                    <div className="mt-3 p-3 bg-slate-100 rounded-lg border border-dashed border-slate-300">
                      <p className="text-[9px] text-slate-500 font-bold italic">Cycle: {employee.workingDaysPerMonth || 26}d. OT: ₹{stats.appliedOTRate || 0}/hr.</p>
                    </div>
                </div>
                <div className="bg-sky-600 text-white p-6 rounded-2xl flex flex-col justify-center items-center shadow-lg relative overflow-hidden">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] opacity-80 mb-2 relative z-10">Net Payable Amount</span>
                    <span className="text-4xl font-black tracking-tighter relative z-10">₹{Math.round(stats.netSalary).toLocaleString()}</span>
                    <div className="mt-3 flex items-center gap-1 px-3 py-1 bg-white/20 rounded-full relative z-10">
                      <span className="text-[9px] font-black uppercase tracking-widest">Verified Statement</span>
                    </div>
                </div>
            </div>

            {/* Footer */}
            <div className="mt-10 flex justify-between items-end opacity-60 page-break-inside-avoid pb-4">
                <div className="space-y-1">
                  <p className="text-[9px] font-black uppercase tracking-widest">Audit ID: {Math.random().toString(36).substring(7).toUpperCase()}</p>
                  <p className="text-[9px] font-bold text-slate-400 uppercase">System Generated - No physical signature required</p>
                </div>
                <div className="text-right">
                  <div className="w-40 border-b border-slate-900 mb-2"></div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-900">Authorized Signatory</p>
                </div>
            </div>
        </div>
      </div>
