...
import { calculateSalary, downloadCSV, formatTime12h, formatDate } from './utils.ts';
...
const Reports: React.FC<Props> = ({ employee, records, user }) => {
  ...
  const getDailyAmount = (record: AttendanceRecord) => {
    ...
    const dailySalary = Number(employee.monthlySalary) / workingDays;
  ...
  const exportExcel = () => {
    ...
    monthlyRecords.forEach(r => {
      ...
      csvContent += `${formatDate(r.date)},${inTime},${outTime},${r.status},${(r.totalHours || 0).toFixed(2)},${(r.otHours || 0).toFixed(2)},${Math.round(pay)}\n`;
    });
  ...
  return (
    <div className="space-y-6 pb-24">
              ...
              {monthlyRecords.map(r => {
                ...
                return (
                  <tr key={r.date} className="hover:bg-white/5 transition-colors group">
                    <td className="px-4 py-4 text-xs font-mono text-white/60">{formatDate(r.date)}</td>
      ...
      <div id="print-area" className="hidden print:block fixed inset-0 bg-white text-black z-[9999] p-4">
        <div className="max-w-[210mm] mx-auto bg-white min-h-[297mm] flex flex-col">
            {/* Header - More Compact */}
            <div className="flex justify-between items-start border-b-2 border-slate-900 pb-4 mb-4">
               <div>
                  <h1 className="text-3xl font-black text-slate-950 uppercase tracking-tighter leading-none mb-1">Salary Statement</h1>
                  <div className="flex items-center gap-2">
                    <span className="bg-slate-900 text-white text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-widest">Official Record</span>
                    <span className="text-slate-500 text-[9px] font-bold">Billing Cycle: {selectedMonth}</span>
                  </div>
               </div>
               <div className="text-right">
                  <h2 className="text-xl font-black text-sky-600 tracking-tighter leading-none">ATTENDIFY</h2>
                  <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest">Enterprise Cloud System</p>
                  <p className="text-[7px] text-slate-400 font-bold">Gen: {new Date().toLocaleDateString('en-GB').replace(/\//g, '-')}</p>
               </div>
            </div>

            {/* Profile & Stats Bento - Scaled Down */}
            <div className="grid grid-cols-2 gap-4 mb-4">
               <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-2">Employee Identity</p>
                  <div className="space-y-1">
                    <p className="text-lg font-black text-slate-950 leading-tight">{employee.name}</p>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <p className="text-[7px] font-black text-slate-400 uppercase">Employee ID</p>
                        <p className="text-xs font-bold">{employee.employeeId}</p>
                      </div>
                      <div>
                        <p className="text-[7px] font-black text-slate-400 uppercase">Designation</p>
                        <p className="text-xs font-bold">{employee.role}</p>
                      </div>
                    </div>
                    <div className="pt-1">
                      <p className="text-[7px] font-black text-slate-400 uppercase">Organization</p>
                      <p className="text-xs font-bold text-sky-600 truncate">{user?.companyName || 'N/A'}</p>
                    </div>
                  </div>
               </div>
               <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-2">Attendance Summary</p>
                  <div className="grid grid-cols-2 gap-x-2 gap-y-2">
                     <div>
                        <p className="text-[7px] font-black text-slate-400 uppercase">Cycle Days</p>
                        <p className="text-base font-black leading-none">{employee.workingDaysPerMonth || 26}</p>
                     </div>
                     <div>
                        <p className="text-[7px] font-black text-slate-400 uppercase">Present</p>
                        <p className="text-base font-black text-emerald-600 leading-none">{stats.presentDays}</p>
                     </div>
                     <div>
                        <p className="text-[7px] font-black text-slate-400 uppercase">OT Accrued</p>
                        <p className="text-base font-black text-amber-500 leading-none">{stats.totalOTHours.toFixed(2)}</p>
                     </div>
                     <div>
                        <p className="text-[7px] font-black text-slate-400 uppercase">Absents</p>
                        <p className="text-base font-black text-rose-500 leading-none">{stats.absentDays + stats.halfDays * 0.5}</p>
                     </div>
                  </div>
               </div>
            </div>

            {/* Attendance Ledger Table - Optimized for Single Page */}
            <div className="border border-slate-200 rounded-lg overflow-hidden mb-4 shadow-sm flex-grow">
                <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-100 border-b border-slate-200">
                        <tr>
                          <th className="py-2 px-3 text-[8px] font-black text-slate-600 uppercase tracking-widest">Date</th>
                          <th className="py-2 px-3 text-[8px] font-black text-slate-600 uppercase tracking-widest">In</th>
                          <th className="py-2 px-3 text-[8px] font-black text-slate-600 uppercase tracking-widest">Out</th>
                          <th className="py-2 px-3 text-[8px] font-black text-slate-600 uppercase tracking-widest">Status</th>
                          <th className="py-2 px-3 text-[8px] font-black text-slate-600 uppercase tracking-widest text-center">Worked</th>
                          <th className="py-2 px-3 text-[8px] font-black text-slate-600 uppercase tracking-widest text-center">OT</th>
                          <th className="py-2 px-3 text-[8px] font-black text-slate-600 uppercase tracking-widest text-right">Total ₹</th>
                        </tr>
                    </thead>
                        ...
                        {monthlyRecords.map(r => {
                          ...
                          return (
                              <tr key={r.date} className="hover:bg-slate-50">
                                <td className="py-1 px-3 text-[9px] font-mono font-bold text-slate-500">{formatDate(r.date)}</td>
                                <td className="py-1 px-3 text-[9px] font-bold">{formatTime12h(r.checkIn)}</td>
                                <td className="py-1 px-3 text-[9px] font-bold">{formatTime12h(r.checkOut)}</td>
                                <td className="py-1 px-3">
                                  <span className={`text-[7px] font-black uppercase tracking-tight px-1 py-0.25 rounded ${
                                ...
                                <td className="py-1 px-3 text-[9px] font-bold text-center">{(r.totalHours || 0).toFixed(2)}</td>
                                <td className="py-1 px-3 text-[9px] font-bold text-amber-600 text-center">{(r.otHours || 0).toFixed(2)}</td>
                                <td className="py-1 px-3 text-[9px] font-black text-right text-slate-900">₹{Math.round(dailyTotal)}</td>
                              </tr>
                          );
                        })}
                    </tbody>
            ...
            {/* Earnings Final Card - Compact Layout */}
            <div className="grid grid-cols-2 gap-4 pt-4 border-t-2 border-slate-900 page-break-inside-avoid">
                <div className="space-y-2">
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Breakdown Detail</p>
                    <div className="space-y-1">
                    ...
                    <div className="mt-2 p-2 bg-slate-100 rounded-lg border border-dashed border-slate-300">
                      <p className="text-[7px] text-slate-500 font-bold italic">Cycle: {employee.workingDaysPerMonth || 26}d. OT: ₹{stats.appliedOTRate || 0}/hr.</p>
                    </div>
                </div>
                <div className="bg-sky-600 text-white p-6 rounded-2xl flex flex-col justify-center items-center shadow-lg relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-2 opacity-10">
                      <Wallet size={40} />
                    </div>
                    <span className="text-[8px] font-black uppercase tracking-[0.3em] opacity-80 mb-1 relative z-10">Net Payable Amount</span>
                    <span className="text-4xl font-black tracking-tighter relative z-10">₹{Math.round(stats.netSalary).toLocaleString()}</span>
                    <div className="mt-2 flex items-center gap-1 px-2 py-0.5 bg-white/20 rounded-full relative z-10">
                      <span className="text-[7px] font-black uppercase tracking-widest">Verified Statement</span>
                    </div>
                </div>
            </div>

            {/* Footer - Compact */}
            <div className="mt-8 flex justify-between items-end opacity-60">
                <div className="space-y-0.5">
                  <p className="text-[7px] font-black uppercase tracking-widest">Audit ID: {Math.random().toString(36).substring(7).toUpperCase()}</p>
                  <p className="text-[7px] font-bold text-slate-400 uppercase">System Generated - No physical signature required</p>
                </div>
                <div className="text-right">
