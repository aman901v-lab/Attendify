...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      unsubUsers = onSnapshot(collection(db, "users"), (snapshot) => {
        ...
        const savedSession = localStorage.getItem('attendify_session_v3');
        if (savedSession) {
        ...
        setInitError("Network Timeout: Check your internet connection.");
        setIsLoading(false);
        setIsSyncing(false);
      });
      ...
      setInitError("Critical Initialization Failure.");
      setIsLoading(false);
    }
  ...
  const handleLogin = (user: User) => {
    setCurrentUser(user);
    localStorage.setItem('attendify_session_v3', JSON.stringify(user));
    if (user.accessibleTabs && user.accessibleTabs.length > 0) {
  ...
  const handleLogout = () => {
    setCurrentUser(null);
    localStorage.removeItem('attendify_session_v3');
  };
  ...
  if (initError) return (
    <div className="min-h-screen bg-[#0B0F1A] flex flex-col items-center justify-center gap-6 p-6 text-center">
      ...
      <h2 className="text-xl font-bold text-white mb-2">Connection Problem</h2>
      <button onClick={() => window.location.reload()} className="flex items-center gap-2 px-6 py-3 bg-blue-500 text-white font-bold rounded-xl shadow-lg">
        <RefreshCcw size={18} /> Retry Cloud Sync
      </button>
    </div>
  );
  ...
  if (isLoading) return (
    <div className="min-h-screen bg-[#0B0F1A] flex flex-col items-center justify-center gap-6">
      ...
      <p className="text-blue-500 font-black uppercase tracking-[0.3em] text-[10px] animate-pulse">Establishing Secure Session</p>
    </div>
  );
  ...
  const visibleNavItems = navConfigs.filter(item => 
    currentUser.role === 'admin' || (currentUser.accessibleTabs && currentUser.accessibleTabs.includes(item.id))
  );
