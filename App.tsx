...
import { calculateSalary, toLocalDateString, generateMonthlyRecords } from './utils.ts';
...
const App: React.FC = () => {
      ...
      try {
        const handlePermissionError = (err: any) => {
          ...
          if (err.code === 'permission-denied') {
            setInitError({ 
              message: "Database Permissions Locked or Access Expired. Please verify your Firestore security rules configuration.", 
  ...
  const handleUpdateUser = async (u: User) => {
    try {
      ...
      console.error("User update failed", err instanceof Error ? err.message : String(err));
    }
  };
  ...
  const stats = useMemo(() => {
    ...
    const employeeRecords = records.filter(r => r.employeeId === activeEmployee.id);
    const monthlyRecords = generateMonthlyRecords(year, month, activeEmployee, employeeRecords, settings.holidays);
    return calculateSalary(monthlyRecords, activeEmployee, currentMonth);
  }, [records, activeEmployee, settings.holidays]);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden print:h-auto print:overflow-visible print:block">
      <header className="flex-shrink-0 z-50 glass border-b px-4 py-3 flex items-center justify-between print:hidden">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="text-[9px] font-black uppercase tracking-widest text-sky-400">{currentUser.role}</p>
      ...
      <div className="flex flex-1 relative overflow-hidden print:overflow-visible print:block">
        <aside className="hidden md:flex w-64 glass border-r p-4 flex-col gap-2 z-40 print:hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 pb-32 print:overflow-visible print:p-0 print:m-0 print:block">
          <div className="max-w-5xl mx-auto print:max-w-none print:mx-0">
            ...
            {activeTab === 'reports' && activeEmployee && <Reports employee={activeEmployee} records={records.filter(r => r.employeeId === activeEmployee.id)} user={users.find(u => u.linkedEmployeeId === activeEmployee.id) || currentUser} holidays={settings.holidays} />}
            {activeTab === 'staff' && <StaffManager employees={employees} users={users} globalWeeklyOffs={settings.weeklyOffs} globalWorkingDays={settings.workingDaysPerMonth} onAdd={async (e, u) => { await setDoc(doc(db, "employees", e.id), e); await setDoc(doc(db, "users", u.id), u); }} onUpdate={async (e, u) => { await setDoc(doc(db, "employees", e.id), e, { merge: true }); if(u) await setDoc(doc(db, "users", u.id), u, { merge: true }); }} onDelete={async (id) => { await deleteDoc(doc(db, "employees", id)); const u = users.find(usr => usr.linkedEmployeeId === id); if(u) await deleteDoc(doc(db, "users", u.id)); }} currentUser={currentUser} />}
            {/* Removed onApproveRecord prop to fix type error as it is not defined in LeaveApproval's Props */}
            {activeTab === 'approvals' && <LeaveApproval requests={leaveRequests} employees={employees} onApprove={async (req) => await setDoc(doc(db, "leaves", req.id), { ...req, status: 'Approved' })} onReject={async (id) => { const req = leaveRequests.find(l => l.id === id); if (req) await setDoc(doc(db, "leaves", req.id), { ...req, status: 'Rejected' }); }} />}
            {activeTab === 'settings' && currentUser && <SettingsView settings={settings} onSave={async (s) => await setDoc(doc(db, "config", "global"), s)} currentUser={currentUser} onUpdateUser={handleUpdateUser} onUpdateAvatar={() => fileInputRef.current?.click()} isUploading={isUploading} currentTheme={localTheme} onThemeChange={setLocalTheme} onLogout={handleLogout} />}
          </div>
        </main>
      </div>

      <nav className="md:hidden glass border-t px-2 py-1 flex justify-around z-[60] pb-safe print:hidden">
        {visibleNavItems.map(item => (
