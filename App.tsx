...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Timeout to prevent infinite loading if Firebase fails quietly
    const timer = setTimeout(() => {
      if (isLoading && !initError) {
        setIsLoading(false);
        // We don't set error here, just let AuthView show if users are empty
      }
    }, 5000);
    ...
    try {
      ...
      setInitError("Initialization Error. Versions conflict.");
    }
  ...
  if (initError) return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center gap-6 p-6 text-center">
      ...
      <button onClick={() => window.location.reload()} className="flex items-center gap-2 px-6 py-3 bg-sky-500 text-white font-bold rounded-xl shadow-lg">
  ...
  if (isLoading) return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-sky-500 font-bold uppercase tracking-widest text-[10px]">Booting Attendify Cloud...</p>
    </div>
  );
