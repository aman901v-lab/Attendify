 ...
 * Fixed Rule: 26 Working Days per month.
 * Any Absence/Unpaid leave results in (Monthly Salary / 26) deduction per day.
 * Weekly Offs and Holidays are paid (no deduction).
 */
export const calculateSalary = (records: AttendanceRecord[], employee: Employee) => {
  ...
  records.forEach(r => {
    if (r.status === 'Duty') {
      ...
      halfDays++;
      deductions += dailyRate * 0.5; // Half day deduction
    } else if (r.status === 'Absent' || r.status === 'Unpaid') {
      absentDays++;
      deductions += dailyRate; // Full day deduction
    } else if (['SL', 'PL', 'CL'].includes(r.status)) {
      paidLeavesUsed++; // Paid leaves, no deduction
    }
  });

  const otEarnings = totalOTHours * (employee.otRate || 0);
  const netSalary = (employee.monthlySalary || 0) - deductions + otEarnings;
