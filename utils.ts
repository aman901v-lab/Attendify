...
export const calculateSalary = (records: AttendanceRecord[], employee: Employee) => {
  ...
  records.forEach(r => {
    if (r.status === 'Duty') {
      ...
      halfDays++;
      // Deduct half day salary (Daily Rate / 2)
      deductions += dailyRate * 0.5;
    } else if (r.status === 'Absent' || r.status === 'Unpaid') {
      absentDays++;
      unpaidLeaves++;
      // Deduct full day salary (Daily Rate)
  ...
  // Total Salary = Monthly Base + OT - (Deductions from Absents/Half-days)
