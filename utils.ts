import { AttendanceRecord, Employee, AttendanceStatus, Holiday } from './types.ts';

export const formatTime = (time?: string) => time || '--:--';

export const formatTime12h = (time24?: string): string => {
  if (!time24) return '--:--';
  const [hours, minutes] = time24.split(':').map(Number);
  const period = hours >= 12 ? 'PM' : 'AM';
  const hours12 = hours % 12 || 12;
  return `${hours12}:${String(minutes).padStart(2, '0')} ${period}`;
};

export const toLocalDateString = (date: Date): string => {
  const year = date.getFullYear();
  const month = String(date.getMonth() + 1).padStart(2, '0');
  const day = String(date.getDate()).padStart(2, '0');
  return `${year}-${month}-${day}`;
};

// Strictly format dates as DD-MM-YYYY regardless of input format
export const formatDate = (dateStr?: string): string => {
  if (!dateStr) return '--/--/----';
  
  try {
    // If it's a full ISO string or contains time, extract just the date part
    if (dateStr.includes('T')) {
      dateStr = dateStr.split('T')[0];
    }
    
    // Try split by '-' or '/'
    const parts = dateStr.includes('-') ? dateStr.split('-') : dateStr.split('/');
    
    if (parts.length === 3) {
      // If first part is a 4-digit year (YYYY-MM-DD)
      if (parts[0].length === 4) {
        const [year, month, day] = parts;
        return `${day.padStart(2, '0')}-${month.padStart(2, '0')}-${year}`;
      }
      // If it's already DD-MM-YYYY but maybe with different separator
      return `${parts[0].padStart(2, '0')}-${parts[1].padStart(2, '0')}-${parts[2]}`;
    }
    
    // Fallback: try parsing with Date object
    const date = new Date(dateStr);
    if (!isNaN(date.getTime())) {
      const day = String(date.getDate()).padStart(2, '0');
      const month = String(date.getMonth() + 1).padStart(2, '0');
      const year = date.getFullYear();
      return `${day}-${month}-${year}`;
    }
  } catch (e) {
    console.error("Error formatting date:", e instanceof Error ? e.message : String(e));
  }
  
  return dateStr;
};

export const calculateHours = (inTime: string, outTime: string): number => {
  const [inH, inM] = inTime.split(':').map(Number);
  const [outH, outM] = outTime.split(':').map(Number);
  let diff = (outH * 60 + outM) - (inH * 60 + inM);
  if (diff < 0) diff += 24 * 60; 
  return Math.round((diff / 60) * 100) / 100;
};

export const calculateWorkedHours = (inTime: string, outTime: string): number => {
  const total = calculateHours(inTime, outTime);
  const worked = Math.max(0, total - 0.5);
  return Math.round(worked * 100) / 100;
};

export const getOT = (workedHours: number): number => {
  const ot = Math.max(0, workedHours - 8);
  return Math.round(ot * 100) / 100;
};

export const getAutoStatus = (dateStr: string, employee: Employee, holidays: Holiday[]): AttendanceStatus | null => {
  const isHoliday = holidays.find(h => h.date === dateStr);
  if (isHoliday) return 'Holiday';
  
  const [year, month, day] = dateStr.split('-').map(Number);
  const date = new Date(year, month - 1, day);
  const dayOfWeek = date.getDay();
  
  if (employee.weeklyOffs.includes(dayOfWeek)) return 'Weekly Off';
  return null;
};

export const generateMonthlyRecords = (
  year: number,
  month: number,
  employee: Employee,
  records: AttendanceRecord[],
  holidays: Holiday[],
  fullMonth: boolean = false
): AttendanceRecord[] => {
  const selectedMonth = `${year}-${month.toString().padStart(2, '0')}`;
  
  // Find the last punch date in the selected month
  const recordsForMonth = records.filter(r => r.employeeId === employee.id && r.date.startsWith(selectedMonth));
  const lastPunchDate = recordsForMonth.length > 0
    ? recordsForMonth.reduce((max, r) => r.date > max ? r.date : max, '')
    : '';

  const todayStr = toLocalDateString(new Date());
  const isCurrentMonth = selectedMonth === todayStr.slice(0, 7);
  
  // If there's a punch date, use it as cutoff. Otherwise, if it's the current month, use today.
  // If it's a past month and no punches, show the whole month.
  const cutoffDate = lastPunchDate || (isCurrentMonth ? todayStr : `${selectedMonth}-31`);
  
  const daysInMonth = new Date(year, month, 0).getDate();
  const cutoffDay = parseInt(cutoffDate.split('-')[2], 10);
  const daysToGenerate = fullMonth ? daysInMonth : Math.min(daysInMonth, cutoffDay);

  const today = new Date();
  today.setHours(0, 0, 0, 0);

  return Array.from({ length: daysToGenerate }, (_, i) => {
    const day = i + 1;
    const dateStr = `${year}-${month.toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`;
    
    const existingRecord = records.find(r => r.date === dateStr);
    if (existingRecord) {
      return existingRecord;
    }
    
    const autoStatus = getAutoStatus(dateStr, employee, holidays);
    const recordDate = new Date(dateStr);
    const isFuture = recordDate > today;
    
    return {
      employeeId: employee.employeeId,
      date: dateStr,
      checkIn: '',
      checkOut: '',
      status: autoStatus || (isFuture && !fullMonth ? 'Pending' : 'Absent'),
      notes: '',
      totalHours: 0,
      otHours: 0
    } as AttendanceRecord;
  });
};

export const getActiveSalaryForMonth = (employee: Employee, targetMonth?: string): number => {
  if (!targetMonth) return Number(employee.monthlySalary) || 0;
  
  if (!employee.salaryHistory || employee.salaryHistory.length === 0) {
    return Number(employee.monthlySalary) || 0;
  }
  
  // Sort history chronologically by month
  const sortedHistory = [...employee.salaryHistory].sort((a, b) => a.month.localeCompare(b.month));
  
  let activeSalary = Number(employee.monthlySalary) || 0;
  for (const increment of sortedHistory) {
    if (increment.month <= targetMonth) {
      activeSalary = Number(increment.salary);
    } else {
      break;
    }
  }
  return activeSalary;
};

export const calculateSalary = (records: AttendanceRecord[], employee: Employee, targetMonth?: string) => {
  const workingCycleDays = Number(employee.workingDaysPerMonth) || 26;
  const monthlySalary = getActiveSalaryForMonth(employee, targetMonth);
  const dailySalary = monthlySalary / workingCycleDays;
  const hourlyBaseSalary = dailySalary / 8;
  
  // Custom OT rate from staff profile or default to hourly base
  const otRate = (Number(employee.otRate) > 0) ? Number(employee.otRate) : hourlyBaseSalary;
  
  let totalOTHours = 0;
  let totalDeductionDays = 0;
  let presentDaysCount = 0;
  let absentDaysCount = 0;
  let halfDaysCount = 0;

  records.forEach(r => {
    if (r.status === 'Duty') {
      presentDaysCount++;
      if (r.otHours) totalOTHours += Number(r.otHours);
    } else if (r.status === 'Half-Day') {
      halfDaysCount++;
      totalDeductionDays += 0.5;
      if (r.otHours) totalOTHours += Number(r.otHours);
    } else if (r.status === 'Absent' || r.status === 'Unpaid') {
      absentDaysCount++;
      totalDeductionDays += 1;
    }
  });

  const roundedOTHours = Math.round(totalOTHours * 100) / 100;
  const otEarnings = roundedOTHours * otRate;
  const calculatedDeductions = totalDeductionDays * dailySalary;
  
  // Cap deductions at monthly salary to avoid negative base pay
  const deductions = Math.min(calculatedDeductions, monthlySalary);
  
  const netSalary = monthlySalary - deductions + otEarnings;

  return {
    dailyRate: Math.round(dailySalary * 100) / 100,
    totalOTHours: roundedOTHours,
    otEarnings: Math.round(otEarnings * 100) / 100,
    deductions: Math.round(deductions * 100) / 100,
    netSalary: Math.max(0, Math.round(netSalary)),
    presentDays: presentDaysCount,
    absentDays: absentDaysCount,
    halfDays: halfDaysCount,
    appliedOTRate: Math.round(otRate * 100) / 100,
    monthlySalary: monthlySalary
  };
};

export const downloadCSV = (content: string, fileName: string) => {
  const blob = new Blob([content], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.setAttribute("href", url);
  link.setAttribute("download", fileName);
  link.style.visibility = 'hidden';
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
};