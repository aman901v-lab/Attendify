...
export const calculateWorkedHours = (inTime: string, outTime: string): number => {
  ...
  return parseFloat(worked.toFixed(2));
};
...
export const getOT = (workedHours: number): number => {
  ...
  return parseFloat(ot.toFixed(2));
};
...
export const calculateSalary = (records: AttendanceRecord[], employee: Employee) => {
  ...
  // Rounding total OT hours to prevent long decimals (floating point error)
  ...
  return {
    dailyRate: parseFloat(dailySalary.toFixed(2)),
    hourlyRate: parseFloat(hourlySalary.toFixed(2)),
    totalOTHours: roundedOTHours,
    otEarnings: parseFloat(otEarnings.toFixed(2)),
    deductions: parseFloat(totalDeductionAmount.toFixed(2)),
