...
import { calculateWorkedHours, getOT, getAutoStatus, toLocalDateString } from './utils.ts';
...
const Dashboard: React.FC<Props> = ({ stats, employee, records, holidays, onAddRecord }) => {
  ...
  // Formatting OT Hours to round figure with 2 decimal places for UI as requested
  const displayOTHours = stats?.totalOTHours ? Number(stats.totalOTHours).toFixed(2) : '0.00';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="glass p-4 rounded-2xl border border-white/5 overflow-hidden">
          ...
          <p className="text-2xl font-bold text-emerald-400">{displayOTHours} <span className="text-xs font-normal text-slate-500">Hrs</span></p>
        </div>
        <div className="glass p-4 rounded-2xl border border-white/5">
